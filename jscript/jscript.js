...
function x_each(object, callback) {
    for (var e = new Enumerator(object); !e.atEnd(); e.moveNext()) {
        callback(e.item());
    }
}
function x_map(object, callback) {
    var ret = [];
    for (var e = new Enumerator(object); !e.atEnd(); e.moveNext()) {
        ret.push(callback(e.item()));
    }
    return ret;
}
function x_select(object, callback) {
    var ret = [];
    for (var e = new Enumerator(object); !e.atEnd(); e.moveNext()) {
        if (callback(e.item())) {
            ret.push(e.item());
...
(function () {
    ...
    if (WScript.Arguments.length > 0) {
        ...
        process.argv = x_select(WScript.Arguments, function () { return true; }).slice(1);
    }
    else {
        process.exit(0);
    }
    ...
    var BufferInterface = function (stream) {
        var stream_ = stream || new ActiveXObject("ADODB.Stream");
        this.open = function () {
            stream_.Open();
        };
        this.close = function () {
    ...
    var TextBuffer = function () {
        var stream_ = new ActiveXObject("ADODB.Stream");
    ...
    var DataBuffer = function () {
        var stream_ = new ActiveXObject("ADODB.Stream");
        var buffer_ = new BufferInterface(stream_);
        var dom_ = new ActiveXObject("Msxml2.DOMDocument.6.0");
